...
class delim (base_primitive):
    def __init__ (self, value, fuzzable=True, name=None):
        ...
        confine = [2, 5, 10, 25, 50, 100, 500, 1000, 5000, 10000, 65535]
        if self.value:
            for tmp in confine:
                self.fuzz_library.append(self.value * tmp)
...
class string (base_primitive):
    ...
    def __init__ (self, value, size=-1, padding="\x00", encoding="ascii", fuzzable=True, max_len=0, name=None):
        ...
        [
            ...
            self.value * 500 + "\xfe",
            self.value * 1000 + "\xfe",
            self.value * 10000 + "\xfe",
            self.value * 65535 + "\xfe",
        ]
...
class bit_field (base_primitive):
    ...
    def mutate(self):
        ...
        if len(self.ex_fuzz_list) == 0:
            ...
            for index in xrange(len(self.ex_fuzz_list)):
                ...
                self.ex_fuzz_list[swapIndex],self.ex_fuzz_list[index]
    ...
    def add_integer_boundaries (self, integer):
        ...
        for i in xrange(-10, 10):
            ...
            if self.min_num <= case <= self.max_num:
